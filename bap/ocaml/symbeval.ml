...
let new_unk_byte = 
  ...
  (fun () -> 
    ...
    Ast.Var(Var.newvar ("unk_" ^ idx) Type.reg_8)
  )
      ...
      (fun k v ->
  	 match k,v with
  	   | var, ConcreteMem(mem,_) ->
               ...
               AddrMap.iter
  		 (fun i v ->
  		    pdebug((Printf.sprintf "%Lx" (Big_int_Z.int64_of_big_int i))
  			   ^ " -> " ^ (Pp.ast_exp_to_string v))
  		 )
  		 mem
  	   | _ -> ()
...
struct
  ...
  let read_typed va t = 
    let rd i = 
      ...
      !read_byte (Big_int_Z.add_int_big_int i va)
    in
    let n = match t with
      | Type.Reg 8 -> 1
      | Type.Reg 16 -> 2
  ...
  let rec lookup_mem mu index endian =
    ...
    | ConcreteMem(m,v), Int(i, t) ->
      ...
      (try AddrMap.find norm_addr m
       with Not_found ->
        (try 
          let b = read_typed norm_addr Type.reg_8 in
          Ast.Int(b, Type.reg_8)
...
sig
  ...
  val inst_fetch : t -> Type.addr -> Ast.program * Z.t 
...
struct
  ...
  let inc = Big_int_Z.succ_big_int
  let dec = Int64.pred
  let add = Int64.add
  ...
  let insts2str insts =
    ...
    let pr acc = function
      | Inst(va, stmts) ->
        ...
        in
        let vas = Pr.sprintf "0x%Lx" (Big_int_Z.int64_of_big_int va) in
        (vas, asm)::acc
    in
  ...
  *)
  ...
  let inst_fetch sigma pc =
    ...
    with Not_found ->
      let s = Pr.sprintf "%Lx" (Big_int_Z.int64_of_big_int pc) in
  ...
  let rec eval_and_taint_stmt 
    ...
    let swap_pc0_to_va ip lab = 
      match ip, lab with
      | Pc(pc), Addr addr -> Va(addr)
      | Pc(pc), _ -> failwith "swap_pc0_to_va: unexpected label type"
      | _, _ -> ip
    in
  ...
  let forward_exec start_va reg_vals taint read_byte jd id =
    let make_block ({sigma=sigma;lambda=lambda;ip=ip} as ctx) stmts = 
      ...
      let set_sigma_lambda sigma lambda stmts =
        ...
        let enu_stmts = L.map (fun (s,i) -> (s, Big_int_Z.big_int_of_int i)) enu_stmts in
      ...
      {ctx with ip=Pc(Big_int_Z.zero_big_int);sigma=sigma;lambda=lambda;} 
    in
    let run_block ctx = 
    ...
    let empty_mem = ConcreteMem(AddrMap.empty, Dis.R64.mem) in
    let var_vals = (Dis.R64.mem, empty_mem)::reg_vals in
