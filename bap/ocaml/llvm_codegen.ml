...
class codegen ?(opts=true) memimpl =
  ...
  let () =
IFDEF LLVM_3_4_API THEN
    Llvm_target.DataLayout.add_to_pass_manager the_fpm (ExecutionEngine.data_layout execengine)
ELSE
IFDEF LLVM_3_2_API THEN
...
(** Convert a cfg to llvm IR. *)
  method convert_cfg cfg =
    ...
    let add_labels bb stmts =
      (* Don't add the same address twice, even if there are multiple
         labels *)
      let seenh = Hashtbl.create (CA.G.nb_vertex cfg) in
      List.iter (function
        | Label (l, _) ->
          (match exp_of_lab l with
          | Int _ as e when Hashtbl.mem seenh e = false ->
            Hashtbl.add seenh e ();
