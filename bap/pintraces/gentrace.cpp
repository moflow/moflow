...
void PIN_FAST_ANALYSIS_CALL LogBlockHit(THREADID tid, ADDRINT address)
{
  ...
  PIN_GetLock(&g_bb_lock, tid+1);
  it = g_bbs.find(address);
  if(it != g_bbs.end()){
    PIN_ReleaseLock(&g_bb_lock);
    return;
  }
...
*/
    ...
    if (abort && !SEHMode.Value()) {
        ...
        ADDRINT gsp = PIN_GetContextReg(ctx, LEVEL_BASE::REG_STACK_PTR);
        PIN_SetContextReg(ctx, LEVEL_BASE::REG_STACK_PTR, gsp + STACK_OFFSET);
    ...
    for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins)) {
        ...
        for(uint32_t i = 0; i < INS_OperandCount(ins); i++) {
            ...
            if (INS_OperandIsReg(ins, i)) {
                ...
                if (fullr != LEVEL_BASE::REG_INVALID() && fullr != r) {
