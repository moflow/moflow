...
void test_tlv_triage(char *buf)
{
  ...
  if((unsigned int)h->record_count > 8)
	return;

  while(count < (unsigned int)h->record_count)
  {
    printf("count: %d, record_count %d\n", count, h->record_count);
    printf("record: type %d, len %d, val %p\n", r->type, r->len, r->val);
    if(r->type == 1)
    {
      memcpy(newbuf + offset, r->val, r->len); // readav?
    }
    else if(r->type == 2)
    {
